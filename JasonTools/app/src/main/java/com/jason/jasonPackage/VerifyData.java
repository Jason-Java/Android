...
public class VerifyData implements IVerifySerialProtocolData {
    ...
    private IProtocol sendVerifyData(IProtocol protocol, int i) throws VerifyFailedException {
        if (JasonVerifyUtil.crc8_Maxim(protocol.getProtocol(), 1, 3) != protocol.getProtocol()[4]) {
        ...
        //箱子地址
        String hex = StrUtil.byteToHexString(protocol.getProtocol()[1]);
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(0, 1), 16));
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(1, 2), 16));
        //钥匙地址
        hex = StrUtil.byteToHexString(protocol.getProtocol()[2]);
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(0, 1), 16));
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(1, 2), 16));
        //命令号
        hex = StrUtil.byteToHexString(protocol.getProtocol()[3]);
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(0, 1), 16));
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(1, 2), 16));
        //校验
        hex = StrUtil.byteToHexString(protocol.getProtocol()[4]);
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(0, 1), 16));
        bytes[index++] = (byte) (0x30 + Integer.parseInt(hex.substring(1, 2), 16));
        //结束
    ...
    private IProtocol receiveVerifyData(IProtocol protocol, int len) throws VerifyFailedException {
        ...
        if (JasonVerifyUtil.crc8_Maxim(bytes, 1, bytes.length - 3) != bytes[bytes.length - 2]) {
            throw new VerifyFailedException("接受数据crc校验失败");
        }
        protocol.setProtocol(bytes);
        LogUtil.i(TAG, "解析完成的数据 " + StrUtil.byteToHexString(bytes));
        return protocol;
    }
}
